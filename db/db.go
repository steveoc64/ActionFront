...
func CreateGameData(gameData *db.Col) {
	...
	// Corps Orders
	gameData.Insert(DataMap("CorpsOrder", CorpsOrder{"Attack", []string{"Attack", "Bombard", "Support", "March"}, "At least 1 ME must attempt to engage."}))
	gameData.Insert(DataMap("CorpsOrder", CorpsOrder{"Engaged", []string{"Attack", "Bombard", "Defend", "Support", "March", "Rest", "Redeploy", "BreakOff"}, "At least 1 ME must attempt to stay engaged."}))
	gameData.Insert(DataMap("CorpsOrder", CorpsOrder{"Defend", []string{"Defend", "Support", "March", "Rest", "Redeploy", "BreakOff"}, "At least 1 ME must have a Defend order"}))
	gameData.Insert(DataMap("CorpsOrder", CorpsOrder{"Manoeuvre", []string{"Support", "March", "Screen"}, "The Corps will march to a new location"}))
	gameData.Insert(DataMap("CorpsOrder", CorpsOrder{"Withdraw", []string{"Rearguard", "Defend", "Support", "March", "Rest", "BreakOff"}, "The Corps must try to have most of its MEs disengaged until it reaches the destination"}))
	...
	gameData.Insert(DataMap("MEOrder", MEOrder{"Support", "The ME is to support another ME", "Intercept order may be activated when enemy is within 3 grids", true, false, true, true, false}))
